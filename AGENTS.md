...
| Tests | **2566 pass** — 951 API + 275 ai + 272 domain + 805 web + 149 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
Verified working: lint 9/9, typecheck 9/9, 2566 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 152 entries, each saying what the failure looks like |
...
**The full list — 152 entries in 10 groups, each written to say what it looks like when it goes wrong —
