...
| Tests | **1103 pass** — 349 API + 114 domain + 131 web + 149 nlp + 108 rules-engine + 258 ai (+contracts) |
...
| Known gap → task 2.2.5 | **`captureCommit` is built; duplicate-suspect detection is not.** docs/06 §5.2.2's third mechanism (the ±2-day amber "looks like a duplicate" chip, plus the undo toast) is 2.2.5. Idempotency (I-10) and offline dedupe **did** land with 2.2.4: a row whose `idempotencyKey` or `clientId` already exists short-circuits before validation and returns `wasReplayed = true`, with the partial unique indexes as the authority. `DashboardDelta` (docs/06 §5.2.3) is deliberately not built — the client refetches `dashboard`, and the success payload carries `reviewQueueCount` and `cursor` instead; the SDL deviations are recorded in docs/06 §5.2.4. Also deferred: no AI provider is configured, so `AI_CLASSIFIER` resolves to the honest always-unavailable classifier (rules-only, `degraded: true`) until provider config lands — the router and adapters in `packages/ai` are already built. `classification_decisions.prompt_template_id` is `null` because no `prompt_templates` seed exists; `prompt_version` **is** recorded. The per-household threshold override is **read** from `households.settings`, but the mutation that writes it plus its `audit_log` row is not built |
...
Verified working: lint 9/9, typecheck 9/9, 1103 tests, `web:build`, GraphQL over HTTP through the
