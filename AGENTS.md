...
| Tests | **3158 pass** — **1255** API + 285 ai + **283** domain + **1063** web + **158** nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none). Both counts moved in the ADR-040 i18n pass; `toCyrillic`'s specs moved from nlp to domain; the web's moved with `/notifications`' on-demand check (3.1.4) then with profile/verification/2FA and the settings shell (0.6.4–0.6.7: +64 API, +39 web) |
...
Verified working: lint 9/9, typecheck 9/9, 3158 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
