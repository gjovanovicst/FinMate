...
| Tests | **2824 pass** — **1101** API + 275 ai + 272 domain + **910** web + 152 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
| Known gap → Phase 3 assistant | **`assistantAnswer` is live** (closed intent registry → pure planner → fact assembly → `NARRATE` → numeric validator, with `TEMPLATE_FALLBACK` and an honest refusal), and **so are three writes** through [ADR-035](docs/14-decisions-and-risks.md)'s propose→confirm registry: `ADD_CATEGORY`, `ADD_TRANSACTION` and `SET_BUDGET`, each reachable from `/assistant` by a question and each executable only by a click. **The A-series is recorded in docs/06 §8.9–§8.15 and the write path in §8.16; the battery that gates all of it is docs/10 §5.9 — read those, not this row.** Still open, each in docs/06 §8.8: `conversationId`; narration cost persistence; a merchant/tag drill-through; **F-30 applies nothing**; English-only refusal and action copy while the cards are bilingual; narration **never evaluated**; `kirija`/English seed content; an income **schedule** question; **A-13b**, the both-scopes product decision. Writes still to come: `ADD_GOAL`, `ADD_TAG` ([docs/16](docs/16-assistant-context-and-actions.md) B-4b/c) and `CREATE_RULE_FROM_CORRECTION` (B-5). ⚠️ **A budget is create-only** (no operation restores the amount an overwrite would replace) and ⚠️ **`VIEWER` is still unenforced** on the taxonomy writes. |
...
Verified working: lint 9/9, typecheck 9/9, 2824 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
