...
- **4.3.1d measured every screen instead of assuming, and it paid twice.** A browser instrument walked all
  ...
  (two inactive controls are **exempt**, WCAG 1.4.3). It also **measured** the tap-target gap: **17
  controls sit under WCAG's 24 px floor** and 37 more under this repo's 44 px rule — open as **4.3.1e**.
...
- **4.3.4 put the bundle budgets in CI and measured accessibility with axe.** **4.3.4a**: `pnpm
  ...
  file sandbox) → deferred CI job. Four traps recorded in docs/15.
- **4.3.2a made the app installable, checked against Chrome rather than the files.** A manifest, four
  generated icons (`pnpm icons:generate`, a dependency-free PNG encoder; maskable inside Android's safe
  zone, `apple-touch-icon` for iOS, which ignores the manifest's icons) and both in the worker's asset
  group. CDP `Page.getInstallabilityErrors` returns **none**, the worker is `activated`, and each icon's
  PNG header was decoded. **4.3.2b** is the funnel (docs/07 §4.7). ⚠️ The brand now lives in static assets
  too, so a rename is a search over five places (docs/15).
- **Next**: **4.3.2b** — the install funnel; then the **human visual pass** (4.3.1e's 17 controls under
  24 px, 4.3.1c's pinned capture bar, the nested-`<main>` landmarks), 4.3.3 and 4.3.4b's Lighthouse job.
...
| Not yet built | **the password-reset and email-verify screens** — the API flow works and its mail links to `/reset-password`, which is not a route (docs/09 5.8); the remaining background jobs; production build for apps/api (its own decision); the **install-promotion funnel** — the app became installable in 4.3.2a (manifest, icons, Chrome-verified), but nothing yet invites a user to do it (4.3.2b); the dashboard's **pending strip** (the query selects no pending count; the queue's own count is the header chip); and the review-queue route for a re-classified offline row (it needs its own `ReviewReason` arm) |
...
Verified working: lint 9/9, typecheck 9/9, 2579 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 158 entries, each saying what the failure looks like |
...
**The full list — 158 entries in 10 groups, each written to say what it looks like when it goes wrong —
