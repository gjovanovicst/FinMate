...
- **Phase 4.2 offline & sync**: the service worker — `ngsw`, **app-shell-only with no `dataGroups`**,
  ...
  fresher than they are (4.2.8a), and then **the screens that serve it** (4.2.8b): `/transactions` writes
  the cache after a successful **unfiltered** read and serves it, labelled and read-only, when the read
  fails — the record carries the ledger currency, a filtered read is neither cached nor served, and a
  cached row claims nothing the whitelist does not hold (no tap, no review flag, no category where
  `null` could mean "divided") — **ADR-027's 4.2.8b amendment**; and **ADR-031**, which repaired AI residency: an `*_EU` endpoint must
  ...
  composed seam.
- **Next**: **2.2.7** (the model's category vs the deterministic direction — docs/04 §8.1.6), the **consent sheet** (docs/02 §4.18's
  *AI podešavanja*; R-25 — until it exists a Household can only be asked through the mutation, so
  every other Household gets rules and keywords), 4.3 (mobile polish), Phase 5 (hardening and beta), and
  the human visual pass at 320/768/1280 px that **no screen has had** — Phase 1's own gate, still open.
...
| Tests | **2456 pass** — 931 API + 274 ai + 272 domain + 716 web + 149 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
| Not yet built | the remaining background jobs; production build for apps/api (its own decision); a **web app manifest and icons** — so the PWA is still not *installable* even though the worker ships (4.3.2, blocked on the undecided product name, ADR-014); **analytics' offline view** — the only 📖 row in docs/07 §6 with no record behind it, and an open decision (docs/09's 4.2.8b note says "analytics' cached period follows the same rule" while docs/07 §6 and ADR-027's rejected option (f) say it needs its own design); the dashboard's **pending strip** (the query selects no pending count; the queue's own count is the header chip); and the review-queue route for a re-classified offline row (it needs its own `ReviewReason` arm) |
...
| Known gap → **found by 2.2.6's live verification** | **The model's category and the deterministic direction are never reconciled.** Reproducing a user's own entry — `Lidl 2000, fuel 3500, salary 150000`, English words, so the rules found nothing and the model was asked — showed the model answering well on categories (`fuel`→`Gorivo` 0.807, `salary`→`Plata` 0.765) and both rows saving with `kind = EXPENSE`, while `Plata` is an **INCOME** Category. `PipelineCategory` carries `kind`, and `packages/ai` states the reconciliation rule for the *amount* explicitly; the direction has no equivalent. It is the mirror of docs/04 §8.1.5 — there a stage decided while the direction was unknown (fixed by a gate that refuses), here the direction is known and a later stage contradicts it — and it lands in the verify lane at 0.765, so nothing flags it. Scheduled as **task 2.2.7**, because every fix (filter the candidate list by direction / flip `kind` / drop the category and ask / set 8.1.5's direction flag) is a product decision about which side wins. docs/04 §8.1.6 |
...
Verified working: lint 9/9, typecheck 9/9, 2456 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 125 entries, each saying what the failure looks like |
...
**The full list — 125 entries in 10 groups, each written to say what it looks like when it goes wrong —
