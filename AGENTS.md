...
| Tests | **1236 pass** — 433 API + 114 domain + 174 web + 149 nlp + 108 rules-engine + 258 ai (+contracts) |
...
| Known gap → the rest of Sprint 2.3 | **2.3.1 is complete; 2.3.2a (the review queue's API) is done.** Still to come: **2.3.2b the `/review` screen** — the queue, the badge and the nav move (the badge and the screen are the whole point of F-08, and until they exist a blocking-lane row is invisible to the user), then **2.3.3 the F-13 seed** (short and mislocated, see above), then **2.3.4 embeddings** (which is also what makes F-09's shorthand work). Also unbuilt and documented: `bulkResolveReviewItems` (`applyToSimilar` covers the affordance); `ReviewItemKind.RECEIPT_ITEM` and three of five `ReviewReason` arms (no producer); the `CONFIDENCE`/`AMOUNT` queue sorts (a keyset page on a non-unique key repeats or skips rows — it needs a composite cursor); the rule **backfill**; the `acceptProposal: false` refusal is not persisted; the `/rules` screen cannot edit a rule's conditions; no **Restore** for an undone capture; the undo's `audit_log` entry (Phase 5); `DashboardDelta`. No AI provider is configured, so `AI_CLASSIFIER` is the honest always-unavailable classifier (rules-only, `degraded: true`), and `classification_decisions.prompt_template_id` is `null` while `prompt_version` **is** recorded. The per-household threshold override is read from `households.settings.aiConfidenceThresholds`, but the mutation that writes it plus its `audit_log` row is not built |
...
Verified working: lint 9/9, typecheck 9/9, 1236 tests, `web:build`, GraphQL over HTTP through the
