...
| Tests | **1050 pass** — 316 API + 114 domain + 105 web + 149 nlp + 108 rules-engine + 258 ai |
...
| Known gap → task 2.2.4/2.2.5 | **`captureCommit` is not built, and `captureParse` is the only entry point the classification module exposes.** docs/06 §5.2's write half (atomic multi-row validate → classify → persist, `clientId`/`idempotencyKey` dedupe, duplicate-suspect detection) is tasks 2.2.4–2.2.5, so `classification_decisions.transaction_id` is still always `null` and `ClassificationService.attachToTransaction` is the seam the ledger will call. Also deferred: no AI provider is configured, so `AI_CLASSIFIER` resolves to the honest always-unavailable classifier (rules-only, `degraded: true`) until provider config lands — the router and adapters in `packages/ai` are already built. And `classification_decisions.prompt_template_id` is `null` because no `prompt_templates` seed exists; `prompt_version` **is** recorded. `classification_thresholds` overrides are **read** from `households.settings`, but the mutation that writes them plus its `audit_log` row is not built |
...
Verified working: lint 9/9, typecheck 9/9, 1050 tests, `web:build`, GraphQL over HTTP through the
