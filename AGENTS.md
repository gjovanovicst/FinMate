...
receipts COMPLETE, 4.2 offline & sync nearly so (4.2.1–4.2.4 and 4.2.9 done, 4.2.5a is the push ADR).**
...
- **Phase 4.2 offline & sync**: the service worker — `ngsw`, **app-shell-only with no `dataGroups`**,
  ...
  surface and the honest `reasons` a skipped row carries (4.2.9).
- **Next**: 4.2.5 (the client subscription — permission asked after an explanation, and the
  `/notifications` note derived from `pushPublicKey`), then 4.2.6 (the app lock),
  4.2.7 (queued edits and the money-field diff), 4.2.8 (the ledger-rows cache), 4.3 (mobile polish),
...
| Tests | **2286 pass** — 895 API + 272 domain + 598 web + 149 nlp + 108 rules-engine + 258 ai + 6 worker (+contracts) |
...
Verified working: lint 9/9, typecheck 9/9, 2286 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 109 entries, each saying what the failure looks like |
...
**The full list — 109 entries in 10 groups, each written to say what it looks like when it goes wrong —
