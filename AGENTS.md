...
| Tests | **2670 pass** — **987** API + 275 ai + 272 domain + **873** web + 149 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
| Known gap → Phase 3 assistant | **`assistantAnswer` is live** (docs/06 §4.4, §8.7–§8.10): planner → fact assembly → `NARRATE` → numeric validator → answer, with `TEMPLATE_FALLBACK` when a numeral is unaccounted for or no provider is reachable, and a **refusal** (`answered: false`, no figure, the narrator never called) for anything the registry cannot answer. `NARRATOR` comes from the AI composition root; the dev `.env` routes it to `DEEPSEEK_GLOBAL` (verified live: `narrationMode: LLM`, ~1.2 s, 117 micros; withdrawing consent drops the same question to `TEMPLATE_FALLBACK`/`CONSENT_DECLINED`). **A-1/A-2 (2026-09-17)**: every facts total is a signed `Balance`, so a negative net, trend change, budget remaining, account balance or projection overrun renders instead of answering a 500, and the numeric validator compares **signs** as well as digits (docs/06 §8.9); and the four templates that answered `NOT_BUILT` are built from `GoalsService`/`RecurringService`, the services `/goals` and `/recurring` already read (docs/06 §8.10). **Deliberately not built**, each in docs/06 §8.8: `conversationId` (no conversation store); narration **cost persistence** (docs/05 §3's claim that it writes `classification_decisions` was wrong); a drill-through for merchant- or tag-scoped answers; **F-30 computes but applies nothing** (the *Primeni* button is an unmade product decision); the fallback and refusal copy is English (§5.14's no-catalogue breach); the planner matches at most 200 Merchants/Accounts; and narration is **never evaluated** (`test:evals`'s narration gates are `skipped`). The read-coverage and write-action design is [docs/16](docs/16-assistant-context-and-actions.md) (write actions need ADR-035 / Q-11). One planner defect found and fixed: a Household's own Merchant now beats the global seed row of the same name (`NamedEntity.owned`). |
...
Verified working: lint 9/9, typecheck 9/9, 2670 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
