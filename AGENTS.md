...
| Tests | **2934 pass** — **1180** API + 285 ai + 272 domain + 925 web + 158 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
| Known gap → Phase 3 alerts | **All four channels are wired end to end, server and client** (`IN_APP`, `EMAIL` → Mailhog verified live, and `PUSH`/`WEB_PUSH` through the `WEB_PUSH` seam with an **inert default** when no VAPID pair is configured, so rows stay `QUEUED` for the reason `AlertDispatchModel.reasons` states rather than silently). The payload carries a `notification` block because `ngsw-worker.js` shows nothing without a `notification.title` — ADR-028's amendment — and its only text is `APP_NAME`, so **no amount or name can reach a lock screen**. The client half (`core/push/`, six device states, permission behind a button) is 4.2.5, and on this deployment `pushPublicKey` is null so the panel honestly renders `SERVER_OFF`. `updateNotificationPreferences` stores in `households.settings.notifications`; the settings screen is 3.1.4. ⚠️ Notification copy is **English-only and server-rendered** — the API has no i18n catalogue — a DoD breach recorded in docs/06 §5.14, and `locale` is stored and unhonoured; the brand lives in two places until a shared constant lands. **Not built**: `notificationReceived` (no pub/sub transport; the badge polls), and `BUDGET_THRESHOLD`/`GOAL_REACHED` have no producer. **docs/06 §5.14 and docs/09 own the detail** (4.2.5/4.2.9, 3.4.3/3.4.4). |
...
| Known gap → Phase 3 assistant | **`assistantAnswer` is live** (closed intent registry → pure planner → fact assembly → `NARRATE` → numeric validator, with `TEMPLATE_FALLBACK` and an honest refusal), and **so are six writes** through [ADR-035](docs/14-decisions-and-risks.md)'s propose→confirm registry: `ADD_CATEGORY`, `ADD_TRANSACTION`, `SET_BUDGET`, `ADD_GOAL`, `ADD_TAG` and `CREATE_RULE_FROM_CORRECTION`, each reachable from `/assistant` by a question and each executable only by a click. **The A-series is recorded in docs/06 §8.9–§8.15 and the write path in §8.16; the battery that gates all of it is docs/10 §5.9 — read those, not this row.** Still open, each in docs/06 §8.8: `conversationId`; narration cost persistence; a merchant/tag drill-through; **F-30 applies nothing**; English-only refusal and action copy while the cards are bilingual; narration **never evaluated**; `kirija`/English seed content; an income **schedule** question; **A-13b**, the both-scopes product decision. The write series is complete; the one action docs/16 B.3 names that is still unbuilt is `ADD_RECURRING_RULE`, which no row ever scheduled. ⚠️ **A budget is create-only** (no operation restores the amount an overwrite would replace) and ⚠️ **a goal gets no deadline** from the assistant (relative dates have no parser) and ⚠️ **`VIEWER` is still unenforced** on the taxonomy writes. |
...
Verified working: lint 9/9, typecheck 9/9, 2934 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
