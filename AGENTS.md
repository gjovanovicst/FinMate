...
- **ADR-036 opens the assistant's language chapter** (owner's requirement, 2026-09-18: any language, many
  ...
  which is also what gates the *claim* behind a precision floor. Plan:
...
| Tests | **2905 pass** — **1168** API + 279 ai + 272 domain + **920** web + 152 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
Verified working: lint 9/9, typecheck 9/9, 2905 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 195 entries, each saying what the failure looks like |
...
**The full list — 195 entries in 10 groups, each written to say what it looks like when it goes wrong —
