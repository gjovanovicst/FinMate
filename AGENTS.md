...
| Tests | **2805 pass** — **1088** API + 275 ai + 272 domain + **904** web + 152 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
| Known gap → Phase 3 assistant | **`assistantAnswer` is live**: closed intent registry → pure planner → fact assembly → `NARRATE` → numeric validator, with `TEMPLATE_FALLBACK` and an honest **refusal** (no figure; the narrator is never called) for anything the registry cannot answer. **The A-series (2026-09-17) is recorded in docs/06 §8.9–§8.15 and gated in docs/10 §5.9 — read those, not this row.** In short: signed `Balance`s and a sign-aware validator; the four builders that answered `NOT_BUILT`; one ordered multilingual cue list ([ADR-019](docs/14-decisions-and-risks.md) amendment); a **58-question battery** gating `test:evals` *and* the fast suite in **both directions** (**54/58 answerable**); and the taxonomy fixes — keywords matched below names with the case-ending rung disabled (A-5/A-9), `x`→`ks` folded with **no data migration** ([04 §8.1.7](docs/04-categorization-and-ai-engine.md)), the keyword rung **whole-word** (A-12), and the scope label following the **routed slot** rather than the first filled one (A-13a). **Not built**, each in docs/06 §8.8: `conversationId`; narration cost persistence; a merchant/tag drill-through; **F-30 applies nothing**; English-only refusal copy while the suggestions are Serbian; narration **never evaluated**. Open: `kirija`/English seed content; an income **schedule** question (`dueSoon` lists EXPENSE rules only); **A-13b**, the both-scopes product decision. **Writes are decided, not open**: [ADR-035](docs/14-decisions-and-risks.md) records Q-11 — propose only, **every write confirmed by a click**, no confidence fast path, proposals in Redis with a short TTL; B-1 (that ADR) and **B-2a** are done — a closed action registry, a `text` slot the planner takes from the raw question, a Redis proposal store consumed atomically, `assistantProposeAction` — a `Mutation` since B-3a, because proposing a transaction runs the classifier — and `assistantExecuteAction` (the only write, taking just the proposal id), and `ADD_CATEGORY` end to end (docs/06 §8.16) — and **B-2b built the proposal card** on `/assistant`: offered only when the ledger **refused** the question, it renders the backend's sentence and field diff, offers a `kind` toggle gated on the server's own `defaulted` flag, confirms with the proposal id plus one idempotency key and nothing else, and offers an Undo that calls the same `deleteCategory` the screen beside it calls (docs/06 §8.16). **B-3a added `ADD_TRANSACTION`**, which inherits the capture path rather than re-implementing it — `ClassificationService.parse` at propose, `TransactionsService.captureCommit` at execute with the decision the card showed, so the pipeline runs once and the Category approved is the Category stored — and carries structured preview rows (the amount as `Money` for `fm-money`), fills the account and the direction only when the text did not state them, and refuses rather than guesses on an ambiguous amount, several entries, or no account. **B-3b is its card**: the rows with their amounts through `fm-money`, the Category and the day, the review note, the account picker — and the per-action result half, which is where the browser pass found a confirmed entry offering no Undo at all (the client's undo registry knew only the Category's operation). |
...
Verified working: lint 9/9, typecheck 9/9, 2805 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 179 entries, each saying what the failure looks like |
...
**The full list — 179 entries in 10 groups, each written to say what it looks like when it goes wrong —
