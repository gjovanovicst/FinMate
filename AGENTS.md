...
| Tests | **2722 pass** — **1036** API + 275 ai + 272 domain + **873** web + 152 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
| Known gap → Phase 3 assistant | **`assistantAnswer` is live** (docs/06 §4.4, §8.7–§8.15): planner → fact assembly → `NARRATE` → numeric validator → answer, with `TEMPLATE_FALLBACK` when a numeral is unaccounted for or no provider is reachable, and a **refusal** (`answered: false`, no figure, the narrator never called) for anything the registry cannot answer. `NARRATOR` comes from the AI composition root; the dev `.env` routes it to `DEEPSEEK_GLOBAL` (verified live: `narrationMode: LLM`, ~1.2 s, 117 micros; withdrawing consent drops it to `TEMPLATE_FALLBACK`/`CONSENT_DECLINED`). **The A-series (2026-09-17) is recorded in docs/06 §8.9–§8.15 and gated in docs/10 §5.9**: facts totals are signed `Balance`s, so a negative net/trend/budget/balance renders instead of a 500, and the validator compares **signs**; the four `NOT_BUILT` templates answer from `GoalsService`/`RecurringService`; the cue vocabulary is multilingual in **one ordered rule list** ([ADR-019](docs/14-decisions-and-risks.md) amendment) with English period phrases; the **battery** (58 questions, frozen context) is a gate in `test:evals` *and* the fast suite, checking that every question behaves as declared in both directions; the planner matches `INCLUDE` `CategoryKeyword`s below names, and a spend question scoped to an INCOME Category refuses rather than answering `0,00 RSD`. Battery: **54/58 answerable**. **A-5/A-9/A-10** made the planner read the taxonomy it is handed: `INCLUDE` `CategoryKeyword`s match below names (with the case-ending rung **disabled** for keywords, or a keyword like `zarada` matches the verb `zaradio`), a spend question scoped to an INCOME Category refuses rather than answering `0,00 RSD`, `INCOME_BY_CATEGORY` scopes income to a Category, and `foldForMatching` folds `x` to `ks` so `Maksiju` reaches `Maxi` — a fold change that needs **no data migration** because every reader re-folds stored folded text (docs/04 §8.1.7). ⚠️ **But the shipped seed first answered that question with the `Supermarket` Category, not the Merchant**: a `CategoryKeyword` matched *inside* an inflected word (the exact tier was a substring test), `hasCategory` won the route while `scopePhrase` named the Merchant — a Category total under a Merchant's label (ADR-017) — and the battery's frozen context, which omitted the seed's `maxi`/`lidl` keywords, could not see it. **A-12** made the keyword rung **whole-word/whole-phrase** (`containsSequence`, no case-ending rung; names keep both), so live `Maksiju`/`Lidlu` now answer `SPEND_BY_MERCHANT` with `scope: at Maxi`/`at Lidl`; the fixture context **now carries** those keywords, and the price (`na benzina` refuses, `na goriva` still resolves) is asserted. The remaining design question — a question naming **both** a Category and a Merchant outright — is **A-13**: its **label** half is fixed (**A-13a**, docs/06 §8.15 — the phrase follows the routed template's own slot, so *"na hranu u Lidlu"* no longer prints the Category's total as *"at Lidl"*), and what such a question should *answer* (the intersection, the Merchant, or an ambiguity refusal) is **A-13b**, a product decision that needs the owner ([docs/16](docs/16-assistant-context-and-actions.md)). **Deliberately not built**, each in docs/06 §8.8: `conversationId` (no conversation store); narration **cost persistence** (docs/05 §3's claim that it writes `classification_decisions` was wrong); a drill-through for merchant- or tag-scoped answers; **F-30 computes but applies nothing**; the fallback/refusal **copy** is English-only while the suggestions are Serbian (§5.14's breach); the planner matches ≤200 Merchants/Accounts; narration is **never evaluated** (`test:evals`'s narration gates are `skipped`). Remaining measured gaps: seed content for `kirija`/English; a *schedule* question about income still refuses because `dueSoon` lists EXPENSE rules only. Write actions need **ADR-035 / Q-11** ([docs/16](docs/16-assistant-context-and-actions.md)). |
...
Verified working: lint 9/9, typecheck 9/9, 2722 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 172 entries, each saying what the failure looks like |
...
**The full list — 172 entries in 10 groups, each written to say what it looks like when it goes wrong —
