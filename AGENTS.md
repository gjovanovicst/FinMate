...
- **Phase 4.2 offline & sync**: the service worker — `ngsw`, **app-shell-only with no `dataGroups`**,
  ...
  composed seam.
- **Next**: **the first-use consent sheet** (task 5.2a — the open half of R-25), the **human visual pass** at 320/768/1280 px that no screen has
...
| Tests | **2488 pass** — 944 API + 274 ai + 272 domain + 735 web + 149 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
Verified working: lint 9/9, typecheck 9/9, 2488 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 132 entries, each saying what the failure looks like |
...
**The full list — 132 entries in 10 groups, each written to say what it looks like when it goes wrong —
