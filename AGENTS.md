...
receipts COMPLETE, 4.2 offline & sync nearly so (4.2.1–4.2.4 done, 4.2.5a is the push ADR).**
...
- **Phase 4.2 offline & sync**: the service worker — `ngsw`, **app-shell-only with no `dataGroups`**,
  ...
  with the sender (4.2.9) named as the half docs/09 had not scheduled — **ADR-028** (4.2.5a).
- **Next**: 4.2.9 (the `WEB_PUSH` sender), then 4.2.5 (the client subscription), 4.2.6 (the app lock),
...
| Tests | **2265 pass** — 874 API + 272 domain + 598 web + 149 nlp + 108 rules-engine + 258 ai + 6 worker (+contracts) |
...
Verified working: lint 9/9, typecheck 9/9, 2265 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 107 entries, each saying what the failure looks like |
...
**The full list — 107 entries in 10 groups, each written to say what it looks like when it goes wrong —
