...
| Tests | **2696 pass** — **1013** API + 275 ai + 272 domain + **873** web + 149 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
| Known gap → Phase 3 assistant | **`assistantAnswer` is live** (docs/06 §4.4, §8.7–§8.10): planner → fact assembly → `NARRATE` → numeric validator → answer, with `TEMPLATE_FALLBACK` when a numeral is unaccounted for or no provider is reachable, and a **refusal** (`answered: false`, no figure, the narrator never called) for anything the registry cannot answer. `NARRATOR` comes from the AI composition root; the dev `.env` routes it to `DEEPSEEK_GLOBAL` (verified live: `narrationMode: LLM`, ~1.2 s, 117 micros; withdrawing consent drops the same question to `TEMPLATE_FALLBACK`/`CONSENT_DECLINED`). **A-1/A-2 (2026-09-17)**: every facts total is a signed `Balance`, so a negative net, trend change, budget remaining, account balance or projection overrun renders instead of answering a 500, and the numeric validator compares **signs** as well as digits (docs/06 §8.9); the four templates that answered `NOT_BUILT` are built from `GoalsService`/`RecurringService`, the services `/goals` and `/recurring` already read (docs/06 §8.10); **A-3** closed the cue gaps a measured battery found (docs/06 §8.11): four phrasings each paired with what it must *not* catch (A-3a), then the English cues **and English period phrases** with the [ADR-019](docs/14-decisions-and-risks.md) amendment that records why one ordered rule list carries both languages rather than a per-locale table (A-3b — and it made `hasUnresolvedScope` bilingual, without which an English scoped question answered the month's total). **A-4** turned that battery into a gate: `apps/api/src/evals/fixtures/assistant-questions.json` — 57 questions with a frozen planner context, each declaring the intent it must route to and whether it is answerable — checked by `pnpm test:evals` **and** by `planner-gate.spec.ts` in the fast suite (the planner is pure). The strict gate is *every question behaves as declared, in both directions*; the answered share (51/57, 89.5 %) is a floor. It earned its place before it was written: writing it found that `koliko sam potrošio na benzin` returned a **pharmacy's** total (the stem rung matched `Apoteka Benu` on `ben`) and that `how much did I spend on netflix` returned the **subscription list** (a rule name outranked an explicit spend question) — both fixed, both with regression tests, and both taking the answered count **down** (52 → 51), which is why the gate is not "answer more questions". The six remaining gaps are **one mechanism plus one fold**: four need the planner to match `CategoryKeyword`s rather than names only (the classifier already does, so `benzin`→`Gorivo` works there and not here), one is the `x`↔`ks` fold pair (`Maxi`/`Maksiju`, shared with the classifier), and two need an income-scoped-by-Category template. A-4 also **measured and rejected** the fuzzy second planner rung docs/16 proposed: nearest-exemplar scoring got **3 of 12 held-out colloquial questions right and 9 wrong** (margins 0.006–0.083, i.e. noise), because a lexical score is dominated by the boilerplate two questions share. The refusal's suggestions were fixed structurally instead — what the ledger can say about the entity named, then the canonical questions *filtered to the ones this Household can answer*, asserted so a chip cannot lead to a second refusal. **Deliberately not built**, each in docs/06 §8.8: `conversationId` (no conversation store); narration **cost persistence** (docs/05 §3's claim that it writes `classification_decisions` was wrong); a drill-through for merchant- or tag-scoped answers; **F-30 computes but applies nothing** (the *Primeni* button is an unmade product decision); the fallback and refusal copy is English (§5.14's no-catalogue breach); the planner matches at most 200 Merchants/Accounts; and narration is **never evaluated** (`test:evals`'s narration gates are `skipped`). The read-coverage and write-action design is [docs/16](docs/16-assistant-context-and-actions.md) (write actions need ADR-035 / Q-11). One planner defect found and fixed: a Household's own Merchant now beats the global seed row of the same name (`NamedEntity.owned`). |
...
Verified working: lint 9/9, typecheck 9/9, 2696 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 168 entries, each saying what the failure looks like |
...
**The full list — 168 entries in 10 groups, each written to say what it looks like when it goes wrong —
