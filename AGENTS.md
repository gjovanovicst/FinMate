...
| i18n | `core/i18n/`: **English primary**, Serbian latin + cyrillic. Runtime catalogue (no rebuild), `TranslationKey` derived from `en`, `sr-Cyrl` generated at runtime by `toCyrillic` **from `packages/nlp`** (shared with the API, which transliterates its own server-rendered copy). Language switcher in the shell; document titles are `route.*` keys read by `LocalizedTitleStrategy`. The API's server-composed copy lives in `apps/api/src/common/i18n/` — **ADR-040** |
| Tests | **3039 pass** — **1191** API + 285 ai + 272 domain + **1008** web + **169** nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none). The API's and web's counts moved in the ADR-040 i18n pass; the rest are unmoved |
