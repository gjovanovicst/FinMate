...
| Tests | **2028 pass** — 805 API + 264 domain + 438 web + 149 nlp + 108 rules-engine + 258 ai + 6 worker (+contracts) |
| Worker | `apps/worker` **boots and is scheduled** (ADR-022, task 3.4.1): four BullMQ jobs over the API's own services, `nx run worker:serve`. `insights.generate` runs the **whole daily pipeline** — generate *and* evaluate — since 3.4.4, so a scheduled pass turns its insights into notifications; `notifications.dispatch` is the per-minute drain. The other eight jobs in docs/05 §8's table are unbuilt, and ADR-022 makes stating what makes a job idempotent a precondition for adding one |
...
Verified working: lint 9/9, typecheck 9/9, 2028 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 92 entries, each saying what the failure looks like |
...
**The full list — 92 entries in 10 groups, each written to say what it looks like when it goes wrong —
