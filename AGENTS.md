...
| Tests | **1176 pass** — 410 API + 114 domain + 137 web + 149 nlp + 108 rules-engine + 258 ai (+contracts) |
...
| Known gap → task 2.3.1b and the rest of Sprint 2.3 | **The learning loop's API is complete; it has no UI.** `correctTransaction` (category/merchant/counterparty/amount, `kind` refused on purpose), rule synthesis with the conflict guardrail, `createRuleFromCorrection`, the `rules`/`rule` queries and `updateRule`/`deleteRule` all exist and are tested — but a user cannot reach any of it: the transaction detail sheet has no "Zapamti za ubuduće" affordance and there is no `/rules` screen. That is **2.3.1b**. Then 2.3.2 the review queue UI (rows still land in the blocking lane with no screen), 2.3.3 the F-13 seed (short and mislocated, see above), 2.3.4 embeddings. Also unbuilt: the rule backfill (`applyToExisting`/`backfillPreview`), which rewrites `category_id` on N Transactions and needs its own correctness story; the `acceptProposal: false` refusal is **not persisted** (`corrections` has no column for it); the undo's `audit_log` entry is not written (the `AuditInterceptor` and the hash chain are Phase 5 items); `DashboardDelta` is unbuilt by decision. No AI provider is configured, so `AI_CLASSIFIER` is the honest always-unavailable classifier (rules-only, `degraded: true`). `classification_decisions.prompt_template_id` is `null`; `prompt_version` **is** recorded. The per-household threshold override is read from `households.settings.aiConfidenceThresholds`, but the mutation that writes it plus its `audit_log` row is not built |
...
Verified working: lint 9/9, typecheck 9/9, 1176 tests, `web:build`, GraphQL over HTTP through the
