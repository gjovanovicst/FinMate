...
| Web screens | **All 20 routes ship** — the dashboard (incl. its **offline mode**, ADR-027), `/transactions` (filters, edit sheet, correction with *Zapamti za ubuduće*, CSV, and 4.2.8b's **ledger cache**), `/transactions/:id`, `/capture` (+ the 5.2a first-use consent sheet), `/review`, `/pending`, `/budgets`, `/goals`, `/recurring`, `/analytics`, `/assistant` (the write card since B-2b–B-5), `/notifications`, `/receipts`, `/receipts/:id`, `/categories`, `/merchants`, `/counterparties`, `/tags`, `/rules`, `/settings`, `/onboarding`, the auth screens, and the install sheet. **docs/02 is canonical for what each one does and every recorded deviation from its wireframe** — this row does not restate them, because that is how it reached 3.5 KB. Two screens have had **no human pass at any width**: `/analytics` and `/assistant` (docs/02 §9). |
...
Verified working: lint 9/9, typecheck 9/9, 3054 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
