...
- **ADR-036 opens the assistant's language chapter** (owner's requirement, 2026-09-18: any language, many
  ...
  vocabulary (Q-9, Q-16), and *words-as-data* (numbers, dates, currencies) stays Serbian-shaped until C-3,
  which C-3 has now **measured**: routing precision **21/21**, traps **5/5**, end-to-end 65 % (the gap is *values* — C-5). Plan:
  [16 Part C](docs/16-assistant-context-and-actions.md); risk: **R-30**.
...
| Tests | **2912 pass** — **1175** API + 279 ai + 272 domain + **920** web + 152 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
Verified working: lint 9/9, typecheck 9/9, 2912 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 196 entries, each saying what the failure looks like |
...
**The full list — 196 entries in 10 groups, each written to say what it looks like when it goes wrong —
