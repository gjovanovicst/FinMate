...
- **Phase 4.2 offline & sync**: the service worker — `ngsw`, **app-shell-only with no `dataGroups`**,
  ...
  composed seam.
- **Next**: 4.2.8b (the screens that serve the ledger cache), the **consent sheet** (docs/02 §4.18's
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 120 entries, each saying what the failure looks like |
...
**The full list — 120 entries in 10 groups, each written to say what it looks like when it goes wrong —
