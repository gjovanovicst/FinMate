...
| Tests | **1348 pass** — 457 API + 141 domain + 235 web + 149 nlp + 108 rules-engine + 258 ai (+contracts) |
| Not yet built | worker jobs; production build for apps/api (its own decision); PWA service worker (Phase 4) |
| Known gap → task 2.3.3 | **2.3.3a landed the seed content and the onboarding API; the wizard UI (2.3.3b) is next.** The content now ships from `packages/domain/src/seed/` — **39** category nodes (35 expense, 4 income) with 131 distinct keywords, and **62** merchants (the spec says ~40 and ~60; the old build had 38 and 38). Two facts are worth knowing: **(1) the classifier still cannot resolve a global seed merchant.** `loadContext` filters `merchants WHERE household_id`, so the `household_id IS NULL` rows are readable through the API but invisible to classification; docs/02 §4.1 promised the opposite for a skipped step 4 and that promise is now corrected. Fixing it is not "delete the filter" — a Household that edited a shipped merchant owns a copy-on-write duplicate, so the context would hold two entities with one name and resolution would depend on row order; that needs a precedence rule and its own tests. Onboarding step 4 avoids the question by creating the Household's own rows (with the shipped aliases) through copy-on-write. **(2) The demo Household's own merchants carry default categories**, which is why it never revealed the keyword-weight bug below — a *merchant default* is a stage that always decides. Original note: F-13's seed was **short and mislocated** — docs/01 F-13 and docs/11 §2.3 specify ~60 local merchants, `apps/api/prisma/seed.ts` defined 38, and docs/11 §2.3 said the content lived in `packages/domain/seed/`, which did not exist. Do not "fix" docs/11's ~60 to a smaller number: the spec is the target |
...
Verified working: lint 9/9, typecheck 9/9, 1348 tests, `web:build`, GraphQL over HTTP through the
