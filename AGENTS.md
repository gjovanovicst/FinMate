...
| Tests | **1207 pass** — 410 API + 114 domain + 168 web + 149 nlp + 108 rules-engine + 258 ai (+contracts) |
...
| Known gap → the rest of Sprint 2.3 | **2.3.1 is complete end to end.** Still to come, in docs/09's order: **2.3.2 the review queue UI** (rows land in the blocking lane, the nav has no badge, and there is still no screen for them — the single biggest visible gap), **2.3.3 the F-13 seed** (short and mislocated, see above), **2.3.4 embeddings** (which is also what makes F-09's shorthand work — see the F-09 row). Also unbuilt and documented: the rule **backfill** (`applyToExisting`/`backfillPreview`), which rewrites `category_id` on N Transactions and needs its own correctness story; the `acceptProposal: false` refusal is **not persisted** (`corrections` has no column for it); the `/rules` screen edits `isActive` and deletes, but does **not** yet edit a rule's conditions in the UI (`updateRule` accepts them, so the conflict-recovery flow "edit the rule that wins" currently ends at a link); there is no **Restore** affordance for an undone capture (the row is soft-deleted and reachable only through the API); the undo's `audit_log` entry is not written (Phase 5); `DashboardDelta` is unbuilt by decision. No AI provider is configured, so `AI_CLASSIFIER` is the honest always-unavailable classifier (rules-only, `degraded: true`), and `classification_decisions.prompt_template_id` is `null` while `prompt_version` **is** recorded. The per-household threshold override is read from `households.settings.aiConfidenceThresholds`, but the mutation that writes it plus its `audit_log` row is not built |
...
Verified working: lint 9/9, typecheck 9/9, 1207 tests, `web:build`, GraphQL over HTTP through the
