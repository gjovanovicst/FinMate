...
receipts COMPLETE, **4.2 offline & sync COMPLETE apart from 4.2.8b** (4.2.1–4.2.8a, 4.2.9).**
...
- **Phase 4.2 offline & sync**: the service worker — `ngsw`, **app-shell-only with no `dataGroups`**,
  ...
  composed seam.
- **Next**: **2.2.7** (the model's category vs the deterministic direction — docs/04 §8.1.6), 4.2.8b (the screens that serve the ledger cache), the **consent sheet** (docs/02 §4.18's
...
| Tests | **2446 pass** — 931 API + 274 ai + 272 domain + 706 web + 149 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
| Not yet built | the remaining background jobs; production build for apps/api (its own decision); a **web app manifest and icons** — so the PWA is still not *installable* even though the worker ships (4.3.2, blocked on the undecided product name, ADR-014); the **screens that serve the ledger-rows cache** — `/transactions` and analytics' cached period (4.2.8b; the record itself shipped in 4.2.8a); the dashboard's **pending strip** (the query selects no pending count; the queue's own count is the header chip); and the review-queue route for a re-classified offline row (it needs its own `ReviewReason` arm) |
...
Verified working: lint 9/9, typecheck 9/9, 2446 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 121 entries, each saying what the failure looks like |
...
**The full list — 121 entries in 10 groups, each written to say what it looks like when it goes wrong —
