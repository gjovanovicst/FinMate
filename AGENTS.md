...
| Tests | **2703 pass** — **1020** API + 275 ai + 272 domain + **873** web + 149 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
| Known gap → Phase 3 assistant | **`assistantAnswer` is live** (docs/06 §4.4, §8.7–§8.11): planner → fact assembly → `NARRATE` → numeric validator → answer, with `TEMPLATE_FALLBACK` when a numeral is unaccounted for or no provider is reachable, and a **refusal** (`answered: false`, no figure, the narrator never called) for anything the registry cannot answer. `NARRATOR` comes from the AI composition root; the dev `.env` routes it to `DEEPSEEK_GLOBAL` (verified live: `narrationMode: LLM`, ~1.2 s, 117 micros; withdrawing consent drops it to `TEMPLATE_FALLBACK`/`CONSENT_DECLINED`). **The A-series (2026-09-17) is recorded in docs/06 §8.9–§8.11 and gated in docs/10 §5.9**: facts totals are signed `Balance`s, so a negative net/trend/budget/balance renders instead of a 500, and the validator compares **signs**; the four `NOT_BUILT` templates answer from `GoalsService`/`RecurringService`; the cue vocabulary is multilingual in **one ordered rule list** ([ADR-019](docs/14-decisions-and-risks.md) amendment) with English period phrases; the **battery** (58 questions, frozen context) is a gate in `test:evals` *and* the fast suite, checking that every question behaves as declared in both directions; the planner matches `INCLUDE` `CategoryKeyword`s below names, and a spend question scoped to an INCOME Category refuses rather than answering `0,00 RSD`. Battery: **52/58 answerable**. **Deliberately not built**, each in docs/06 §8.8: `conversationId` (no conversation store); narration **cost persistence** (docs/05 §3's claim that it writes `classification_decisions` was wrong); a drill-through for merchant- or tag-scoped answers; **F-30 computes but applies nothing**; the fallback/refusal **copy** is English-only while the suggestions are Serbian (§5.14's breach); the planner matches ≤200 Merchants/Accounts; narration is **never evaluated** (`test:evals`'s narration gates are `skipped`). Remaining measured gaps: the `x`↔`ks` fold pair, an income-scoped-by-Category template, and seed content for `kirija`/English. Write actions need **ADR-035 / Q-11** ([docs/16](docs/16-assistant-context-and-actions.md)). |
...
Verified working: lint 9/9, typecheck 9/9, 2703 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
