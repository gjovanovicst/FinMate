...
receipts COMPLETE, 4.2 offline & sync nearly so (4.2.1–4.2.6a and 4.2.9 done; 4.2.5a and 4.2.6b are
pending).**
...
- **Phase 4.2 offline & sync**: the service worker — `ngsw`, **app-shell-only with no `dataGroups`**,
  ...
  cross-tab flush mutex ADR-026 deferred here, and the wipe — **ADR-029** (4.2.6a).
- **Next**: 4.2.6b (the app lock's device panel and lock screen — until it ships no install can arm
  one, so the store is still session-only and R-23 stays open), 4.2.7 (queued edits and the
  money-field diff), 4.2.8 (the ledger-rows cache), 4.3 (mobile polish),
...
| Tests | **2351 pass** — 896 API + 272 domain + 662 web + 149 nlp + 108 rules-engine + 258 ai + 6 worker (+contracts) |
...
Verified working: lint 9/9, typecheck 9/9, 2351 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
