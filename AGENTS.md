...
- **And then the login got a profile, a real verification gate and two factors** (owner-requested,
  ...
  and a session list that deliberately names no device because only hashes are stored. `GET /auth/me` now
  ...
  risks **R-35** (lockout) and **R-36** (key loss) name what it costs.
...
| i18n | `core/i18n/`: **English primary**, Serbian latin + cyrillic. Runtime catalogue (no rebuild), `TranslationKey` derived from `en`, `sr-Cyrl` generated at runtime by `toCyrillic` **from `packages/domain`** (shared with the API, which transliterates its own server-rendered copy). It is deliberately **not** in `packages/nlp`: the shell needs it eagerly and docs/07 §11 keeps nlp off the pre-network path. Language switcher in the shell; document titles are `route.*` keys read by `LocalizedTitleStrategy`. The API's server-composed copy lives in `apps/api/src/common/i18n/` — **ADR-040** |
| Tests | **3152 pass** — **1255** API + 285 ai + **283** domain + **1057** web + **158** nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none). The API's and web's counts moved in the ADR-040 i18n pass; `toCyrillic`'s specs moved from nlp to domain with the module; the web's moved again with `/notifications`' on-demand check (3.1.4) and both moved again with profile/verification/2FA (0.6.4–0.6.6: +64 API, +34 web, incl. `profile.service.spec.ts`) |
...
| Web screens | **All 21 routes ship** — the dashboard (incl. its **offline mode**, ADR-027), `/transactions` (filters, edit sheet, correction with *Zapamti za ubuduće*, CSV, and 4.2.8b's **ledger cache**), `/transactions/:id`, `/capture` (+ the 5.2a first-use consent sheet), `/review`, `/pending`, `/budgets`, `/goals`, `/recurring`, `/analytics`, `/assistant` (the write card since B-2b–B-5), `/notifications`, `/receipts`, `/receipts/:id`, `/categories`, `/merchants`, `/counterparties`, `/tags`, `/rules`, `/settings`, **`/profile`** (0.6.4/0.6.6), `/onboarding`, the auth screens, and the install sheet. **docs/02 is canonical for what each one does and every recorded deviation from its wireframe** — this row does not restate them, because that is how it reached 3.5 KB. Two screens have had **no human pass at any width**: `/analytics` and `/assistant` (docs/02 §9); `/profile` and the two-step `/sign-in` are new and have had none either |
| Shell header | **Fixed chrome**: the signed-in shell is exactly one viewport tall and only the **content region** scrolls, so the header and the sidebar stay put and the sidebar's own list scrolls inside it — which is what keeps **Settings** pinned to the bottom of the column at any window height (docs/02 §2.2, ADR-039's amendment). The header holds the unread bell, the language switcher and Sign out; below it sit the **app-update line** (ADR-024, non-dismissible, activates on click) and the **offline chip** — `podaci od <time>` (4.2.4) and `Čeka slanje (n)` → `/pending` (4.2.3) — rendered only when there is something to say. The ⚙ entry opens `/settings`: the app lock, **AI consent** (R-25a — every purpose, its state, the disclosure from the server's own routing table) and a link to `/notifications`; 5.2a's first-use sheet reuses that same card so the disclosure cannot drift. **Not built**: the rest of docs/02 §4.18's sections and the avatar menu — docs/02 §2 says a control that cannot work is *disabled with a tooltip* rather than shown broken. |
...
Verified working: lint 9/9, typecheck 9/9, 3152 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 214 entries, each saying what the failure looks like |
...
**The full list — 214 entries in 10 groups, each written to say what it looks like when it goes wrong —
