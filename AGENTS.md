...
| Tests | **3039 pass** — **1191** API + 285 ai + **283** domain + **1008** web + **158** nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none). The API's and web's counts moved in the ADR-040 i18n pass; `toCyrillic`'s specs moved from nlp to domain with the module |
...
Verified working: lint 9/9, typecheck 9/9, 3039 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
