...
receipts COMPLETE, **4.2 offline & sync COMPLETE apart from 4.2.8** (4.2.1–4.2.7b, 4.2.9).**
...
- **Phase 4.2 offline & sync**: the service worker — `ngsw`, **app-shell-only with no `dataGroups`**,
  ...
  offline edit and the tray panel that explains a refusal (4.2.7b).
- **Next**: 4.2.8 (the ledger-rows cache), 4.3 (mobile polish), Phase 5 (hardening and beta), and the human visual pass at 320/768/1280 px that **no screen has
...
| Tests | **2388 pass** — 896 API + 272 domain + 699 web + 149 nlp + 108 rules-engine + 258 ai + 6 worker (+contracts) |
...
Verified working: lint 9/9, typecheck 9/9, 2388 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
