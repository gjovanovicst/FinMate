...
- **4.3.1d measured every screen instead of assuming, and it paid twice.** A browser instrument walked all
  ...
  at **3.31:1** over **153 element-route pairs**, plus **three anchors with no colour at all** rendering the
  UA `#0000EE` at **2.02:1** — both fixed at the class level (`styles.css`), after which **0 contrast
  failures and 0 overflow** remain (the two inactive controls are correctly **exempt**, WCAG 1.4.3). It also
...
- **4.3.6's offline pass found F-26's exit criterion failing; both R-27(a) halves are now fixed.** The harness
  ...
  flush *is* refused by the server, and the tray on the flushing page says so. **(a1, 4.3.6a)** the store
  backing could not follow the lock: `OfflineStoreHolder` had no `invalidate()` (though the app lock's doc
  named one), so the queue stayed on the in-memory backing and a record that did reach IndexedDB was not read
...
- **Next**: **4.3.7b** — `/assistant` must say how an answer was produced (4.3.7a closed the consent half:
  `aiEgress` no longer discloses a task no code calls — `PARSE` stays routed but is logged instead — and the
  card prints one sentence per destination instead of the same DEEPSEEK/non-EEA sentence twice,
  [ADR-034](docs/14-decisions-and-risks.md)). Then **the human visual pass** — the contact sheet 4.3.1d
  produced (all 20 screens at three widths plus the light theme), which feeds **4.3.1e** (17 controls under
  WCAG's 24 px floor) and **4.3.1c** (the pinned capture bar); 4.3.3 (mobile keyboard) / 4.3.4 (bundle +
  Lighthouse); 4.3.2 (install prompt) stays blocked on the product name.
...
| Tests | **2573 pass** — 955 API + 275 ai + 272 domain + 808 web + 149 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
Verified working: lint 9/9, typecheck 9/9, 2573 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 153 entries, each saying what the failure looks like |
...
**The full list — 153 entries in 10 groups, each written to say what it looks like when it goes wrong —
