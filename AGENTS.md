...
receipts COMPLETE, **4.2 offline & sync COMPLETE in the core** (4.2.1–4.2.6b, 4.2.7a, 4.2.9; 4.2.7b and
4.2.8 remain).**
...
- **Next**: 4.2.7b (the sheet queues an offline edit and the tray renders its conflict), 4.2.8 (the
  ledger-rows cache), 4.3 (mobile polish), Phase 5 (hardening and beta), and the human visual pass at 320/768/1280 px that **no screen has
...
| Tests | **2381 pass** — 896 API + 272 domain + 692 web + 149 nlp + 108 rules-engine + 258 ai + 6 worker (+contracts) |
...
Verified working: lint 9/9, typecheck 9/9, 2381 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 114 entries, each saying what the failure looks like |
...
**The full list — 114 entries in 10 groups, each written to say what it looks like when it goes wrong —
