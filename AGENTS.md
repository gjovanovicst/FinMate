...
| Tests | **3147 pass** — **1255** API + 285 ai + **283** domain + **1052** web + **158** nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none). The API's and web's counts moved in the ADR-040 i18n pass; `toCyrillic`'s specs moved from nlp to domain with the module; the web's moved again with `/notifications`' on-demand check (3.1.4) and both moved again with profile/verification/2FA (0.6.4–0.6.6: +64 API, +29 web) |
...
Verified working: lint 9/9, typecheck 9/9, 3147 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
