...
- **Phase 4.1 receipts**: presigned uploads with an inert scanner seam and `files.purge` (4.1.1); the
  ...
  mismatch screen, plus the `/transactions/:id` drill-in docs/02 §2.1 always listed (4.1.4b/4.1.5); and
  **4.1.6 made the reader real** — the OCR task had been routed since 2.2.1 with no deployment able to serve
  it (ADR-037, ADR-038, R-31).
...
- **5.8 closed the last broken user flow: the emailed links land on screens.** `/reset-password` (no token
  ...
  ⚠️ Both gaps 5.8 named were **closed by 0.6.5** (`email_verified_at` now gates behind
  `REQUIRE_EMAIL_VERIFICATION`, and `POST /auth/resend-verification` exists) and `email_verified_at` is
  read by the guard and by `/profile`.
- **ADR-036 opens the assistant's language chapter** (owner's requirement, 2026-09-18: any language, many
  ...
  EEA-or-local, never a method/URL/id/amount/date. **C-1 and C-2 shipped it** — the task, the seam, both
  planners — and it is **live**: *"Wie viel habe ich diesen Monat ausgegeben?"* answers `SPEND_TOTAL` from
  the ledger, a German command proposes a real `ADD_TAG`, and *"loesche alle meine Transaktionen"* is
  refused because no member means that. With no `ROUTE` endpoint (the default) or no consent, the assistant
  is byte-for-byte what it was — ⚠️ which is the honest limit: declining AI keeps the Serbian/English
  vocabulary (Q-9, Q-16). **C-3 measured the rung** (27-question fixture set, five languages: routing
  precision **25/25**, traps **5/5** — one fixture declares both correct members of an ambiguous sentence, so
  the floor rests on the other 24) and **C-5 fixed the other half** — the *values*, because routing a
  sentence buys only its intent and the amount, date, currency and direction are still read by hand-written
  tables that were Serbian-only while the app ships English first: `salary 85000` was an **expense**,
  `coffee 3.50 today` left `today` in the description, `5 euros` claimed **no currency**, and a routed
  `ADD_TRANSACTION` dropped the numeral its builder reads the amount out of. End-to-end **19/22 (86.4 %)**;
  what is left is **entity** vocabulary, not values (C-4). ⚠️ Two of C-3's numbers were wrong and are
  corrected — the 65 % counted the traps in its denominator, and the German refusal it blamed on *values*
  is entity resolution. Plan: [16 Part C](docs/16-assistant-context-and-actions.md); risk: **R-30**.
- **And then the login got a profile, a real verification gate and two factors** (owner-requested,
  ...
  **0.6.6** is [ADR-041](docs/14-decisions-and-risks.md): TOTP (RFC 6238, in-repo on
  ...
  `POST /auth/login/mfa` completes it — whose code step shipped broken; fixed, with a real-submit spec (docs/15). The TOTP secret is AES-256-GCM ciphertext under an optional
  `MFA_ENCRYPTION_KEY` (absent ⇒ `totpAvailable: false`, said out loud rather than stored in the clear),
  ten 80-bit recovery codes are shown once, and every factor change re-authenticates. The QR is
  `qrcode-generator` (typed, zero deps; [ADR-042](docs/14-decisions-and-risks.md));
...
| Schema | 39 tables + 44 CHECK constraints + 20 partial indexes applied; `_prisma_migrations` current (the newest adds `mfa_challenges` + `mfa_recovery_codes` and the three `users` factor columns, 0.6.6) |
...
| Auth (0.6) | REST `/auth/*`: signup, login, refresh **with rotation + theft detection**, logout, verify, reset, **profile + sessions** (0.6.4), **re-send + an opt-in verified-email gate** (0.6.5) and **two-factor authentication** — TOTP + emailed codes with recovery codes (0.6.6, [ADR-041](docs/14-decisions-and-risks.md)); argon2id; login throttling; `TenantContext` now resolved from a real session |
...
| GraphQL (0.7) | Code-first; `Money` / **`Balance`** (the signed counterpart, for any figure *derived* rather than typed) / `UUID` / `LocalDate` scalars; keyset pagination on the UUIDv7 key; `apps/api/schema.gql` generated as a reviewable artifact. First vertical slice: Accounts, with a backend-computed balance |
...
| i18n | `core/i18n/`: **English primary**, plus Serbian latin + cyrillic and, since **ADR-044**, German, Spanish, French and Arabic. The locale set is **open data** — `LOCALES` is a registry carrying `tag`, both labels and **`direction`** — so adding a language is a catalogue module, one registry entry and one loader line, not a type change in two packages. `TranslationKey` is derived from `en`, which is the **only eager catalogue**; every other is a **dynamic import** (`LOADERS`), because five catalogues in the shell would have broken docs/07 §11 (it was at 97 % of 156 KB). `I18nService.init()` is awaited by an app initializer so a lazy chunk never flashes English, `setLocale` is therefore **async**, and `documentElement.dir` follows the locale (Arabic is RTL; the stylesheet was already logical-property-only). `sr-Cyrl` is generated at runtime by `toCyrillic` **from `packages/domain`** (shared with the API). Deliberately **not** in `packages/nlp`: the shell needs it eagerly and docs/07 §11 keeps nlp off the pre-network path. Language switcher in the shell; document titles are `route.*` keys read by `LocalizedTitleStrategy`. The API's server-composed copy lives in `apps/api/src/common/i18n/` and is a **locale map** (`CopyMap`), with `resolveCopyLocale` returning the language a tag names instead of collapsing it to `en` — **ADR-040**, **ADR-044**. ⚠️ de/es/fr/ar are machine-assisted and unreviewed (**R-37**), RTL has had no human pass (**R-38**) |
| Tests | **3204 pass** — **1266** API + 285 ai + **297** domain + **1084** web + **158** nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none). The counts moved with the ADR-040 i18n pass, then with **ADR-044/045**: +11 API (signup currency + the stored-locale fix), +21 web (the catalogue registry walked instead of three hardcoded locales, the RTL direction assertion, the signup currency field) and +14 domain (the currency table checked against CLDR, currency-by-region, `Intl.Locale` maximisation). `toCyrillic`'s specs moved from nlp to domain; the web's earlier moved with `/notifications`' on-demand check (3.1.4) then with profile/verification/2FA and the settings shell (0.6.4–0.6.7) |
...
| Receipts (F-14) | `apps/api/src/modules/receipts` **implemented in 4.1.3**: `createReceipt`, `extractReceipt`, `addReceiptItem`/`updateReceiptItem`/`removeReceiptItem`, `reconcileReceipt`, `receipts`/`receipt`. Item categories come from the **same** `ClassificationService.parse` a typed fragment uses (auditable in `classification_decisions`); I-6 lives in `@finmate/domain/src/receipts.ts` with both sides of the tolerance asserted. **OCR became real in 4.1.6** — before it, the task was routed, the seam was built and **no deployment could serve it**: `LOCAL` had a compiled-in vision model and nobody ran one, while `OPENAI_EU` advertised `supportsOcr: true` and was never given a model, so `extractReceipt` answered `AI_UNAVAILABLE:no-provider-configured` forever and the screen did not call it. Now `AI_OCR_MODEL` names the model (**no default** — a cloud endpoint without one is a logged skip, ADR-037), `AI_OCR_TIMEOUT_MS` sizes the budget for a slow local reader, `assembleAi` asks the adapter `supports(task)` **before** it writes a route, `CLOUD_OCR` consent is asked for an image on **every** endpoint but `LOCAL` (ADR-038, docs/08 §6.5), `pnpm dev:ai` runs the sidecar (docs/11 §2.5), and *Read the photo* on `/receipts/:id` reports the answer — the count, or the reason and its machine code. ⚠️ **Measured, and it is the honest limit**: on a 3-core CPU with no GPU, `qwen2.5vl:3b` needs **minutes** per receipt (>5 min at full size, 4 m 18 s at 768 px, and a 512 px run produced 1 272 tokens and no answer), so with the documented 20 s budget every local read is a timeout. Production should route `OCR` to an EEA vision endpoint; the local sidecar is the privacy path, not the fast one (R-31). The **OCR webhook (§9.5)** is still not built. `commitReceipt` and `DETACH_TRANSACTION` are (4.1.4a) and both screens that call them are (4.1.4b/4.1.5, `/receipts` + `/receipts/:id`). `CreateReceiptInput.attachmentId` is **required**, so a Receipt exists only over a photo — the library's capture action is the only way in. `receipts` is a plain list with no `filter`/`totalCount`, so the library reads the first 50 and says `{count} shown, newest first` rather than claiming a total |
...
| Offline store (F-26) | `apps/web/src/app/core/offline/` — **ADR-025**, and **docs/07 §6 is the per-feature matrix**: an encrypted `idb` store (AES-GCM-256, PBKDF2 key wrapping) over `outbox`/`snapshot`/`taxonomy` with an in-memory backing, one root-provided holder, and an ordered idempotent outbox with a pure retry/refusal classifier. ⚠️ **Nothing confidential reaches disk until the app lock is armed** (ADR-029, 4.2.6b) — with no wrapping secret the store runs in memory; since 4.3.6a the holder and the queue follow the unlock (ADR-025's amendment, R-27(a)), and since 4.3.6b the composer caches the taxonomy so an offline capture carries an account. `idb` + `fake-indexeddb` are the only new dependencies. |
...
| Known gap → Phase 2 | **The fold is internally inconsistent for `đ`/`ђ` (verified).** Latin `đ` folds to `d` (hand-patched in 2.1.1 to fix the NFD gap) but Cyrillic `ђ` folds to `dj` (docs/04 §3.1's table). So `rođa` folds to `roda` while `рођа` folds to `rodja`, and the orthography-correct Cyrillic spelling of the canonical F-11 case resolves on the **trigram rung at 0.61 instead of normalized at 0.98** — a verify-lane match where an exact one was available. The other seven script pairs (`č/č`, `ć/ћ`, `š/ш`, `ž/ж`, `lj/љ`, `nj/њ`, `dž/џ`) are consistent; `đ/ђ` is the only broken one. Fix is one rule in `foldForMatching` (fold the digraph `dj`→`d` after transliteration, which does NOT change §3.1's transliteration table — `ђ`→`dj` stays correct for reading) — **but check the blast radius first**: a blanket `dj`→`d` also folds a literal Latin `Djordje`, which is a rung change, not just a `ђ` one. ⚠️ **It needs no stored-data step**, which A-10 measured: every reader re-folds stored folded text (docs/04 §8.1.7), so this is a fold rule plus its ladder tests, not a migration. |
...
| Known gap → Phase 3 alerts | **All four channels are wired end to end, server and client** (`IN_APP`, `EMAIL` → Mailhog verified live, and `PUSH`/`WEB_PUSH` through the `WEB_PUSH` seam with an **inert default** when no VAPID pair is configured, so rows stay `QUEUED` for the reason `AlertDispatchModel.reasons` states rather than silently). The payload carries a `notification` block because `ngsw-worker.js` shows nothing without a `notification.title` — ADR-028's amendment — and its only text is `APP_NAME`, so **no amount or name can reach a lock screen**. The client half (`core/push/`, six device states, permission behind a button) is 4.2.5, and on this deployment `pushPublicKey` is null so the panel honestly renders `SERVER_OFF`. `updateNotificationPreferences` stores in `households.settings.notifications`; the settings screen is 3.1.4. ✅ Notification copy is now **composed in the recipient's language** from a server-side copy pair, resolved from that user's stored `users.locale` — the English-only breach docs/06 §5.14 recorded was closed by **ADR-040**, together with the mail, the assistant's answer templates and chips, and the stored Rule/Household names. ⚠️ Named residuals: rows written before ADR-040 keep their old language (no backfill), seed content is still a Serbian tree, and `classification_decisions.rationale` is still English. The brand lives in two places until a shared constant lands. **Not built**: `notificationReceived` (no pub/sub transport; the badge polls), and `BUDGET_THRESHOLD`/`GOAL_REACHED` have no producer. **docs/06 §5.14 and docs/09 own the detail** (4.2.5/4.2.9, 3.4.3/3.4.4). |
| Known gap → Phase 3 goals | **The goals API and the `/goals` screen ship.** `savingGoals`/`savingGoal` and the five mutations are live (docs/06 §4/§5.7), with progress, the required monthly amount, contributions and status all computed by `@finmate/domain`. **`createTransaction` on `contributeToGoal` is deliberately not built** — a contribution is not a Transaction (docs/02 §4.13) and recording the outflow needs an Account transfer, which the ledger can express (`transfer_peer_id`) and no feature builds. Also not built: `Dashboard.goals` (docs/06 §4.1's `# CP` field) and the `GOAL_REACHED` insight producer (still listed as uninstrumented); the assistant's two goal templates **are** built (A-2, docs/06 §8.10) and read `GoalsService`. `SavingGoal.version` from docs/06's sketch is **omitted rather than invented**: `saving_goals` has no version column, and optimistic concurrency for one model is a migration nobody asked for. |
...
| Known gap → Phase 3 UI (notifications) | **`/notifications` has not been looked at by a human at any width** either. The alert preferences live on that screen rather than in a settings shell, because no `/settings` route exists yet; docs/02 §4.18's information architecture is the target once it does. Push channels are listed, and the device panel beneath them derives its sentence from `pushPublicKey` and the browser's own state (4.2.5) rather than a static string. The screen has no pagination control: it reads the first 50 rows, which is more than the daily cap can produce in a day. **The centre was also unreachable in practice, and that is now fixed**: the producer is a daily 06:00 worker job and nothing in the app invoked it, so with no worker running — or simply before 06:00 — the list was empty in a way identical to "nothing to report". `/notifications` now carries a **Check for alerts** button that calls `runAlerts` + `dispatchNotifications` (the mutations the API already declared, idempotent) and renders the run summary as one honest sentence; its first pass also writes the default alert rules, so the preferences beneath the list become usable. |
| Known gap → Phase 3 assistant | **`assistantAnswer` is live** (closed intent registry → pure planner → fact assembly → `NARRATE` → numeric validator, with `TEMPLATE_FALLBACK` and an honest refusal), and **so are six writes** through [ADR-035](docs/14-decisions-and-risks.md)'s propose→confirm registry: `ADD_CATEGORY`, `ADD_TRANSACTION`, `SET_BUDGET`, `ADD_GOAL`, `ADD_TAG` and `CREATE_RULE_FROM_CORRECTION`, each reachable from `/assistant` by a question and each executable only by a click. **The A-series is recorded in docs/06 §8.9–§8.15 and the write path in §8.16; the battery that gates all of it is docs/10 §5.9 — read those, not this row.** Still open, each in docs/06 §8.8: `conversationId`; narration cost persistence; a merchant/tag drill-through; **F-30 applies nothing**; ~~English-only refusal and action copy while the cards are bilingual~~ (**closed by ADR-040**: answers, refusals, chips and fact labels are composed in the reader's language); narration **never evaluated**; `kirija`/English seed content; an income **schedule** question; **A-13b**, the both-scopes product decision. The write series is complete; the one action docs/16 B.3 names that is still unbuilt is `ADD_RECURRING_RULE`, which no row ever scheduled. ⚠️ **A budget is create-only** (no operation restores the amount an overwrite would replace) and ⚠️ **a goal gets no deadline** from the assistant (relative dates have no parser) and ⚠️ **`VIEWER` is still unenforced** on the taxonomy writes. |
...
| Web screens | **All 21 routes ship** — the dashboard (incl. its **offline mode**, ADR-027), `/transactions` (filters, edit sheet, correction with *Zapamti za ubuduće*, CSV, and 4.2.8b's **ledger cache**), `/transactions/:id`, `/capture` (+ the 5.2a first-use consent sheet), `/review`, `/pending`, `/budgets`, `/goals`, `/recurring`, `/analytics`, `/assistant` (the write card since B-2b–B-5), `/notifications`, `/receipts`, `/receipts/:id`, `/categories`, `/merchants`, `/counterparties`, `/tags`, `/rules`, **`/settings` (the account shell: Account · Security · AI and privacy · Notifications, 0.6.4–0.6.7, with `/profile` redirecting to it)**, `/onboarding`, the auth screens, and the install sheet. **docs/02 is canonical for what each one does and every recorded deviation from its wireframe** — this row does not restate them, because that is how it reached 3.5 KB. Two screens have had **no human pass at any width**: `/analytics` and `/assistant` (docs/02 §9); the settings shell and the two-step `/sign-in` are new and have had none either |
| Shell header | **Fixed chrome**: the signed-in shell is exactly one viewport tall and only the **content region** scrolls, so the header and the sidebar stay put and the sidebar's own list scrolls inside it — which is what keeps **Settings** pinned to the bottom of the column at any window height (docs/02 §2.2, ADR-039's amendment). The header holds the unread bell, the language switcher and Sign out; below it sit the **app-update line** (ADR-024, non-dismissible, activates on click) and the **offline chip** — `podaci od <time>` (4.2.4) and `Čeka slanje (n)` → `/pending` (4.2.3) — rendered only when there is something to say. The ⚙ entry opens `/settings`, the **account shell** — Account, Security (app lock, two-step, sessions), AI and privacy (consent, R-25a) and Notifications — and the account chip opens its Account tab; 5.2a's first-use sheet reuses the same consent card so the disclosure cannot drift. **Not built**: the rest of docs/02 §4.18's sections and the avatar menu — docs/02 §2 says a control that cannot work is *disabled with a tooltip* rather than shown broken. |
...
Verified working: lint 9/9, typecheck 9/9, 3204 tests, `pnpm test:evals` gating green, `web:build` (**app shell 135.3 KB / 156 KB after ADR-044**), GraphQL over HTTP through the
...
pnpm dev:ai         # the opt-in local vision sidecar + its model (ADR-037, docs/11 §2.5)
...
- [ ] **No hardcoded user-facing strings** — every string goes through `I18nService.t()`, and
      **every locale in `LOCALES`** has the key. English is primary and the fallback; ADR-044 opened the
      registry, so `translations.spec.ts` walks it and fails on a missing key, an empty value, a dropped
      placeholder, or a catalogue that is really the English one under another name
...
- **Never put a backtick inside a `template:` or `styles:` literal — not even in a comment — and never
  ...
  fail as a bare parse error or a template error that names the wrong problem. **The client's GraphQL
  documents are template literals too**: a `` ` `` in one of their comments ends the document, which is
  how a card shipped a syntax error twice in one task (B-3b) — `/* GraphQL */` is a comment, not a
  licence.
...
- **No hardcoded user-facing copy, and `fm-money` is the only thing that formats money.** Its accessible
  label's income/expense words are `money.income`/`money.expense` catalogue keys — the Serbian literals
  that used to sit there were fixed with the rest of this pass (ADR-040). **Route `title`s are `route.*`
  translation keys**, rendered by `LocalizedTitleStrategy`; a literal there is invisible to the switcher,
  which is how the browser tab stayed Serbian in English.
