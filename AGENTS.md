...
| Tests | **1142 pass** — 376 API + 114 domain + 137 web + 149 nlp + 108 rules-engine + 258 ai (+contracts) |
...
| Known gap → Sprint 2.3 | **Sprint 2.2 is complete; these are the documented gaps on the capture path.** `captureCommit` and duplicate-suspect detection are built. Still missing, in the order the plan schedules them: 2.3.1 correction capture + "Zapamti za ubuduće" rule synthesis (so a correction is a dead end today — `correctTransaction` in docs/06 §5.3 does not exist), 2.3.2 the review queue UI (rows land in the blocking lane and there is no screen for them), 2.3.3 the F-13 seed (short and mislocated, see above), 2.3.4 embedding-based resolution. Also: the undo's `audit_log` entry is **not** written — docs/06 §3's `AuditInterceptor` and docs/08 §10.3's hash chain are both Phase 5 items; the delete is soft, so nothing is lost. And `DashboardDelta` (docs/06 §5.2.3) stays unbuilt — the client refetches `dashboard`; the SDL deviations are in docs/06 §5.2.4. No AI provider is configured, so `AI_CLASSIFIER` is the honest always-unavailable classifier (rules-only, `degraded: true`); the router and adapters in `packages/ai` are built. `classification_decisions.prompt_template_id` is `null` (no `prompt_templates` seed); `prompt_version` **is** recorded. The per-household threshold override is read from `households.settings.aiConfidenceThresholds`, but the mutation that writes it plus its `audit_log` row is not built |
...
Verified working: lint 9/9, typecheck 9/9, 1142 tests, `web:build`, GraphQL over HTTP through the
