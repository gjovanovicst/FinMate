...
- **Phase 4.2 offline & sync**: the service worker — `ngsw`, **app-shell-only with no `dataGroups`**,
  ...
  composed seam.
- **Next**: the **consent sheet** (docs/02 §4.18's
  *AI podešavanja*; R-25 — until it exists a Household can only be asked through the mutation, so
  every other Household gets rules and keywords), the **human visual pass** at 320/768/1280 px that no screen has
  had (Phase 1's own gate, and 4.3 is where it belongs), 4.3 (mobile polish), then Phase 5 (hardening and beta).
...
| Tests | **2463 pass** — 938 API + 274 ai + 272 domain + 716 web + 149 nlp + 108 rules-engine + 6 worker (plus `contracts`, which ships no specs and passes with none) |
...
Verified working: lint 9/9, typecheck 9/9, 2463 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 128 entries, each saying what the failure looks like |
...
**The full list — 128 entries in 10 groups, each written to say what it looks like when it goes wrong —
