...
- **Next**: 4.2.7 (queued edits and the money-field diff), 4.2.8 (the ledger-rows cache), 4.3
  (mobile polish), Phase 5 (hardening and beta), and the human visual pass at 320/768/1280 px that **no screen has
...
| Tests | **2370 pass** — 896 API + 272 domain + 681 web + 149 nlp + 108 rules-engine + 258 ai + 6 worker (+contracts) |
...
Verified working: lint 9/9, typecheck 9/9, 2370 tests, `pnpm test:evals` gating green, `web:build`, GraphQL over HTTP through the
...
| **debugging something that should work** | **[`docs/15-implementation-gotchas.md`](docs/15-implementation-gotchas.md)** — 113 entries, each saying what the failure looks like |
...
**The full list — 113 entries in 10 groups, each written to say what it looks like when it goes wrong —
