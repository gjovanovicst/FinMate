...
/**
 * The payload builder against T-09 (docs/08 §2.3, ADR-028 decision 4).
 *
 * The shape test pins the contract; the second test is the one that matters — a notification row
 * whose own copy is full of amounts and Merchant names must not be able to put any of it on a lock
 * screen. It is written against the row, not against a list of payload keys, so a field added to the
 * payload later still has to be innocuous.
 */
describe('the web-push payload (T-09, ADR-028 decision 4)', () => {
  ...
  it('is exactly { notificationId, kind, deepLink }', () => {
    const payload = JSON.parse(buildWebPushPayload(base)) as Record<string, unknown>;
  ...
  it('carries no amount and no name, however loud the row is', () => {
    ...
    const serialised = buildWebPushPayload(row);
    // The id is a UUID and legitimately contains digits; everything else must be inert.
    const withoutId = JSON.stringify({
      ...(JSON.parse(serialised) as Record<string, unknown>),
      notificationId: '',
    });

    for (const secret of ['Supermarket', 'Lidl', 'RSD']) {
      expect(withoutId).not.toContain(secret);
    }
    // And nothing numeric at all, which is T-09's own test shape (docs/06 §5.14).
    expect(/\d/.test(withoutId)).toBe(false);
  });

  it('falls back to the client\'s generic sentence when there is no insight behind the row', () => {
    const payload = JSON.parse(
      buildWebPushPayload({ ...base, insightKind: null }),
  ...
  it('routes every kind exactly as the client\'s own deepLinkFor does, plus the no-insight case', () => {
    ...
    // shipped one rather than inventing a second answer (task 4.2.5 reconciles the pair).
