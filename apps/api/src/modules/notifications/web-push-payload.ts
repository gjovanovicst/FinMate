/**
 * The web-push payload — ADR-028 decision 4, threat T-09 (docs/08 §2.3).
 *
 ...
 * sentence, no figure and no free text in the payload, so a future copy change cannot leak one — the
 * `title`/`body` a notification row carries are deliberately **not read** here even though the sender
 * has them.
 *
 * A unit test (`web-push-payload.spec.ts`) hands this builder a row whose title and body are full of
 * amounts and names and asserts neither appears in the serialised payload, which is the only
 ...
 * object so a caller cannot append to it on the way out.
 */
export function buildWebPushPayload(row: WebPushPayloadSource): string {
  const kind = row.insightKind ?? GENERIC_PUSH_KIND;
  const payload: WebPushPayload = {
    notificationId: row.id,
    kind,
    deepLink: pushDeepLink(kind),
  };
