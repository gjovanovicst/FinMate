...
/**
 * Worker tests use **Vitest** with `unplugin-swc`, for the same reason the API's do: NestJS 12 ships
 * ESM-only packages and `Test.createTestingModule` needs **decorator metadata**, which Vitest's
 * default esbuild transform cannot emit (ADR-020).
 *
 * The only difference from the API's config is the project root and the `@finmate/api/*` alias, which
 * the worker needs because it boots the API's modules (ADR-022).
 *
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      '@finmate/api': new URL('../api/src', import.meta.url).pathname,
    },
  },
  test: {
    environment: 'node',
