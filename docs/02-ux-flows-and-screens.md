...
> **Build state (4.3.6c): an offline reload reaches what is on the device.** [ADR-033](14-decisions-and-risks.md) closes R-27(b). The shell has a **third state** beside the lock screen and the app: an install the lock has **unlocked** whose session could not be restored because *nothing answered* renders a small offline shell instead of the routes — one sentence saying the server is unreachable so the session is not restored and the queued captures will be sent once the user is back online and signed in, **two links** to the only destinations that work (`/pending` and `/transactions`' cached ledger), a *Sign in* action, and no navigation. Every other route redirects to the tray, and a session the server **refused** (`401`) stays on `/sign-in` — the classification is one function, `isUnreachable`. Nothing is sent without a session: the flush is skipped without an access token, the tray hides its retry controls rather than offering a button that cannot work (docs/02 §2), and a `401` is retryable rather than the entry's fault, so a queued capture survives a signed-out reconnect instead of being parked as *cannot be sent*. Verified live 13/13 against the production build — see docs/09's 4.3.6 row. ⚠️ The offline app is **two screens**: the dashboard, analytics and the assistant stay unreachable, because their content is the server's aggregates and recomputing them from a cache is forbidden (ADR-001).
...
| **Contrast and targets** | ≥ 4.5:1 text, ≥ 3:1 UI boundaries; targets ≥ 44 × 44 px on `compact`, ≥ 32 px in dense mode with a 44 px hit area. **Measured in 4.3.1d**, across all 20 authenticated routes at 320/768/1280 px and both themes: **text contrast is now clean** (0 failures) after two fixes — `--color-text-subtle` was `#6e6e80`, **3.31:1** on `--color-surface-raised`, used by 153 element-route pairs (hints, row metadata, the role label, account/merchant badges), and **three anchors had no colour at all**, so they rendered the user agent's `#0000EE` at **2.02:1** on `--color-bg`; there is now a global `a { color: var(--color-primary) }`, which is what stops a fourth. Two inactive controls are **exempt** (WCAG 1.4.3) and reported rather than silently skipped. ⚠️ **The target half is not met and is open as 4.3.1e**: **17 distinct controls are under the WCAG 2.2 AA floor of 24 px** (11 of them real buttons 21 px tall — including the consent sheet's *Allow*/*Decline*), and a further 37 are under this row's 44 px house rule. The cause is one shared metric (a `.btn` is ~21 px tall), so the fix is a control-size token plus a density review, not fifteen local edits — it belongs with the human pass, because it changes every screen's rhythm. |
