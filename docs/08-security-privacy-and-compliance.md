...
| **T-09** | Information disclosure | Notification content on a lock screen discloses amounts or third-party names | L4 | I3 | Lock-screen-safe payloads by default — no amounts, no Merchant/Counterparty names; full content in-app only; APNs/FCM in the register (§6.5). **ADR-028 makes it structural for push**: the payload carries `{ notificationId, kind, deepLink }` and a *generic* sentence the client already has translated, so there is no field an amount could be put in | `NOTIFY-PRIVACY-*`; tabletop 1 |
