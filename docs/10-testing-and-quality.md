...
| Client | **Per-route bundle budget: built and in CI** — `apps/web/tools/bundle-budget.mjs` via `pnpm bundle:budget`, cold cost against [07 §11](07-platform-strategy-mobile-desktop.md)'s ceilings, warning at 90 % / failure at 100 %, and a route with no budget entry fails (4.3.4a). **Lighthouse (≥ 90, [09 §6](09-implementation-plan.md)) and `axe-core` are measured locally** against the served production dist, not yet a CI job; Web Vitals from Playwright traces is still unwired (the Playwright dependency is an open decision) |
