...
| Manual create/edit ([F-04](01-product-requirements.md)) | ✅ / ⏳ | **Create** is queued as a whole `captureCommit` batch (4.2.3). **Edit** queues as a version-checked `updateTransaction` in the core (4.2.7a, [ADR-030](14-decisions-and-risks.md)) — the sheet that queues it and the conflict panel that explains a refusal are 4.2.7b, so an offline edit still surfaces as an error until then |
