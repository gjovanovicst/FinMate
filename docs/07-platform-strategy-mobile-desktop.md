...
| Ledger snapshot | 📖 | Last synced state, `as of` labelled. **Built**: the `Dashboard` read model (4.2.4) and, since 4.2.8a, the **ledger-rows** record — the current period plus 45 days, capped at 200 rows, through the same whitelist and the same store, with its own `staleAt` so it cannot make the dashboard's figures look fresher than they are (ADR-025 decision 5). **The screens that would serve it are 4.2.8b**, so `/transactions` is still online-only and neither analytics nor the assistant reads it — the assistant must never answer from a stale snapshot (ADR-027's rejected option (f)) |
