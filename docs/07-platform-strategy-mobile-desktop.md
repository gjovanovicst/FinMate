...
| Analytics ([F-20](01-product-requirements.md)) / Assistant ([F-23](01-product-requirements.md)) | 📖 | Cached views only; a new question needs network — we will not fabricate an answer from a stale snapshot. **Not built**: neither has a snapshot record, so both are online-only with their own error states (ADR-027 records why this is not the dashboard's record). 4.2.8b served `/transactions` from the **ledger-rows** record and left this row alone: the plan's note that "analytics' cached period follows the same rule" contradicts this matrix and ADR-027's rejected option (f), so it is recorded as an open decision rather than resolved by the screen that happened to be nearest |
