...
| 4.3.4 | **Performance — (a) DONE, (b) next.** **(a) The bundle budget gate is built and in CI** (`pnpm bundle:budget` → `apps/web/tools/bundle-budget.mjs`, docs/07 §11): per-route **cold cost** against the documented ceilings, warning at 90 % / failure at 100 %, and a route with no budget entry fails (rule 7 — which found that §11 names 8 routes and the router has 24, so the rest are held to the documented 320 KB total through an explicit list). Two measurement traps are handled and recorded: `ng build` defaults to `development` (2–3× too large, and the tool refuses it), and a chunk's `imports` mix static and dynamic edges (following both made every route's marginal cost zero). Measured: shell 137.9 KB (92 %, warns), capture 159.9, transactions 174.9, every other route 138–178, `packages/nlp` 2.5 KB. **Lazy routes were already done** (`loadComponent` everywhere) and **no images ship** — `public/` is empty and the only `<img>` tags render a presigned receipt photo, so image sizing has nothing to size until the PWA icons exist (4.3.2). **(b) Lighthouse + axe**: measured locally via `npx` against the served production dist, numbers recorded; the CI job is scheduled separately so a shared-runner ≥90 gate cannot flake PRs. Not built and recorded: the per-PR **delta vs `main`** | 2.5 | F-26 |
