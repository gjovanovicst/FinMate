...
| 4.2.8b | **The screens that serve it** — **DONE**: `/transactions` writes the cache after a successful **unfiltered** read and serves it, labelled and read-only, when the read fails; the record carries the ledger currency, and a filtered read is neither cached nor served. **NOT done, and now an open decision**: *analytics' cached period follows the same rule* contradicts [07 §6](07-platform-strategy-mobile-desktop.md)'s matrix and ADR-027's rejected option (f) — see ADR-027's 4.2.8b amendment. The assistant deliberately does **not** read it (rejected option (f)) | 0.5 | F-26, F-24 |
