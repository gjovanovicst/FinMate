...
| 4.2.8b | **The screens that serve it** — `/transactions` writes the cache after a successful read and serves it (labelled) when the read fails; analytics' cached period follows the same rule, and the assistant deliberately does **not** (ADR-027's rejected option (f)) | 0.5 | F-26, F-24 |
