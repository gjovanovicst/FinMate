...
| 4.3.1e | **Control sizes** — **found by 4.3.1d**: 17 distinct controls are under WCAG 2.2 AA's 24 px floor (11 real buttons at 21 px tall, including *Allow*/*Decline* in the consent sheet) and 37 more miss this repo's own 44 px compact rule. One shared metric is the cause, so the fix is a control-size token (44 px compact / 32 px dense) applied to buttons, chips, tabs and icon buttons — **with** the human pass, because it changes every screen's vertical rhythm. A blanket `min-block-size` was deliberately *not* shipped: it would grow the small in-row controls (the capture row's `×`, pager arrows) as well | 1 | F-26, F-05 |
