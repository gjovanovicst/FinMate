...
| **R-27** | **The offline pass found two holes in F-26's exit criterion.** (a) **A flush that meets a 5xx drops the queued entry instead of retrying or refusing it**: measured with the network cut, a queued capture drained on reconnect, the header chip disappeared, the tray read **"0 waiting to send, 0 refused"** — and the database held **zero transactions and zero classification decisions**. (b) **An offline reload cannot restore the session**, so the locked install's re-auth screen is a dead end: reloading offline renders *Unlock the app*, the PIN unlocks it, and the router lands on **`/sign-in`** with no way in while the network is down — the queued work is on disk and unreachable. The exit criterion says "full capture flow works in airplane mode", and after a reload it does not | 4 | 4 | 16 | (a) is silent by construction: nothing is left in the tray and the chip is gone, so the only trace is a row that never existed (R-20's rule is that a capture is never dropped silently). **(a) is now diagnosed.** The flush's request *does* reach the server, and the server **refuses the whole batch**: `{"data":{"captureCommit":{"__typename":"CaptureCommitRejectedModel","code":"VALIDATION_FAILED","message":"1 row(s) could not be committed, so none were. ….accountId: A row with no accountId needs a default …"}}}` — captured by wrapping `fetch` and `XMLHttpRequest` **inside the page**, because the app consumes the body before Playwright can read it. So the offline-queued `CaptureCommitInput` rows omit `accountId`, which the online path supplies; the server will not default it, and nothing is written. **The next question is what the fix is**: either the queued payload must carry the account the composer was using (the screen already holds it), or the server's default must exist for the Household — docs/06 §5.2 decides which. The second half of (a) is presentation: the earlier pass read the tray as *0 waiting to send, 0 refused* while the entry was gone, so a refused **capture** may not be counted by a line that was written for refused **edits** — check that before believing the row is recoverable. The two obvious client-side suspects remain ruled out: `outbox.flush` retries a 5xx correctly, and `GraphqlClient.query` throws for an `errors` body or a 200 with no `data`. | **4.3.6**, before any claim that F-26 works in airplane mode; and before the launch gates, which assert exactly that |
