...
| **R-27** | **The offline pass found two holes in F-26's exit criterion.** (a) **A flush that meets a 5xx drops the queued entry instead of retrying or refusing it**: measured with the network cut, a queued capture drained on reconnect, the header chip disappeared, the tray read **"0 waiting to send, 0 refused"** — and the database held **zero transactions and zero classification decisions**. (b) **An offline reload cannot restore the session**, so the locked install's re-auth screen is a dead end: reloading offline renders *Unlock the app*, the PIN unlocks it, and the router lands on **`/sign-in`** with no way in while the network is down — the queued work is on disk and unreachable. The exit criterion says "full capture flow works in airplane mode", and after a reload it does not | 4 | 4 | 16 | (a) is silent by construction: nothing is left in the tray and the chip is gone, so the only trace is a row that never existed (R-20's rule is that a capture is never dropped silently). **The diagnosis so far rules out the two obvious suspects**, which is why this is still open: `outbox.flush`'s classifier is *correct* for a 5xx (it re-puts the entry as `pending`, increments `attempts` and stops the drain), and `GraphqlClient.query` throws both for an `errors` body **and** for a 200 with no `data`, wrapping an `HttpErrorResponse` with its status. Neither can silently remove an entry. Since only a **resolved** `send` calls `store.remove`, the flush must have received a response that looked like a successful commit — and a probe confirms the flush's `POST /graphql` returned **HTTP 200**. Yet nothing was written. **The next experiment, not a guess**: record the response *body* inside the page (an init script that wraps `fetch`, or a patched `GraphqlClient`), because Playwright cannot read it — the app consumes it — and note that the API's structured log is **boot-only**, so there is currently no server-side way to confirm the request arrived at all. (b) is the documented design: the access token is in memory only (docs/08 §2.1) and `authenticatedGuard` restores it over the network, so the lock's local key is necessary but not sufficient. Fixing (b) means deciding that an unlocked lock may authorise an **offline** session until reconnect — a real security decision (revocation would not apply until then), so it needs an ADR rather than a patch | **4.3.6**, before any claim that F-26 works in airplane mode; and before the launch gates, which assert exactly that |
