...
- ✅ **The device panel and the re-auth screen shipped in 4.2.6b** (`/settings` → `Bezbednost`, and the
  ...
  "F-26's offline capture survives a reload" — is false as measured in 4.3.6**: the backing never follows
  the unlock unless a data screen happens to ask for it, and even a record that reaches IndexedDB is not
  read back after a reload. See R-27(a); the capability is real, the wiring is not. R-23 stays closed on
  the capability (an install that leaves the lock off still persists nothing — a decision, not a gap), and
  its persistence claim is now R-27's.
