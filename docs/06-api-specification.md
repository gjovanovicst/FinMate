...
| The mismatch screen (4.1.4b) | **Not built** | The banner, the item table with its category pickers and the two buttons are the web half of this task; the operations they call are live and verified by the integration suite and a live GraphQL pass. |
...
| `PUSH`/`WEB_PUSH` delivery | **Not implemented** | The browser subscription store and the service worker are Phase 4 (docs/09 §6) and there is no push client dependency. Rows stay `QUEUED` and `dispatch` reports them as `skipped` on every pass — the honest state, and the reason the count does not fall to zero. |
