...
| `extractReceipt` from the screen | **Called since 4.1.6** by *Read the photo* (`/receipts/:id`) | It was deliberately uncalled while no deployment could serve it (ADR-037): every answer was `AI_UNAVAILABLE:no-provider-configured`, and a control that always fails is a control this build cannot honour. Now the operator can name a reader — the local sidecar (`LOCAL_AI_BASE_URL`) or an EEA vision endpoint (`AI_OCR_PRIMARY=OPENAI_EU` + `AI_OCR_MODEL`) — and the screen renders the answer either way: the count on success, the reason and its machine code on a refusal, with manual itemisation unchanged as the fallback. |
...
| Notification copy | **Rendered server-side in the reader's language, since ADR-040** | `notifications.title`/`body` are `TEXT NOT NULL`, so the sentence is composed once and rendered verbatim in-app, by email and by push. `notification-copy.ts` now takes a `CopyLocale` and composes from an `{ en, sr }` pair, and `NotificationsService.run` resolves it from the **recipient's** stored `users.locale`. That closes the breach this row used to record: the copy was English-only while `mail.service.ts` was Serbian-only, so one of the two was always wrong. ⚠️ Rows written before the change keep their old language (no backfill — rewriting a user's stored row on a language change is worse than the inconsistency), and `NotificationPreferencesInput.locale` remains a **preference** the dispatch path does not read; `users.locale` is the column that decides. |
...
| `runAlerts` | A mutation, and the method the daily job calls; **the `/notifications` screen now calls it too** | The worker exists since 3.4.1, and since 3.4.4 `insights.generate` calls the **same** `NotificationsService.run` this mutation does — generate, then evaluate against the rules, in the pipeline's order. Both are idempotent (insight dedupe keys, `notifications.dedupe_key`), and `notifications.dispatch` remains the per-minute drain. Before 3.4.4 the job called `InsightsService.generate` alone, so a scheduled run wrote insight rows that nothing ever turned into a notification. **The screen half was the remaining gap**: nothing in the app invoked it, so a deployment whose worker had not run (or simply the hours before 06:00) showed an empty centre indistinguishable from "nothing to report". `/notifications`' **Check for alerts** button calls this mutation, then `dispatchNotifications`, then reloads, and renders the summary as one sentence (`checkOutcome` in `notifications.view.ts`). It is a button and not a load-time write: the pass writes rows, and a page visit should not. The first pass also writes the Household's default rules, so the preferences beneath the list become usable at the same moment. |
